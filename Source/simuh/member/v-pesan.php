...
$db->go("SELECT tbl_pesan.*, tbl_admin.username, tbl_admin.nama FROM tbl_pesan JOIN tbl_admin ON tbl_pesan.id_sender = tbl_admin.id_admin WHERE id_pesan = '$idp' AND tbl_pesan.id_receiver = '$ids'");
$psn=$db->fetchArray();
$kat=$psn['kat'];
?>
      ...
      <div class="panel panel-default"><?php echo ViewMessage(); ?>
        ...
        <div class="panel-body panel-body-nopadding">
          ...
          <?php
            ...
            $sub = $psn['subject'];
            $bls = $_POST['balasan'];
            if(isset($_POST['balas']) && !empty($bls)){$db->go("INSERT INTO tbl_pesan (id_sender, id_receiver, subject, pesan, kat) VALUES ('$ids','$dari','$sub','$bls','B')"); Message(1, 'Balasan dikirim');Redirect(base_url.'/member/v-pesan.php?id='.$idp);}elseif(isset($_POST['balas']) && empty($bls)){Message(4, 'Balasan gagal/kosong!!!');Redirect(base_url.'/member/v-pesan.php?id='.$idp);}
          ?>
          <div class="form-horizontal form-bordered"> 
            ...
            <div class="form-group">
              ...
              <div class="col-sm-6">
               <strong><?php if($kat=='B'){echo "<span class='label label-warning'>Balasan</span> ".$psn['subject'];}else{echo $psn['subject'];} ?></strong>
              </div>
            </div>
            <div class="form-group">
